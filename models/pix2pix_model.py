...
class Pix2PixModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['G_L11_50','G_GAN_50','D_fake50','D_real50','G_L11_100','G_GAN_100','D_fake100','D_real100','G_L11_150','G_GAN_150','D_fake150','D_real150']
       ...
       # self.netG_E = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.norm,
         #                             not opt.no_dropout, not opt.no_encode, opt.init_type, opt.init_gain, self.gpu_ids)
      #  if self.isTrain:  # define a discriminator; conditional GANs need to take both input and output images; Therefore, #channels for D is input_nc + output_nc
          #  self.netD20 = networks.define_D(opt.input_nc+ opt.output_nc ,  opt.ndf, opt.netD,
        ...
        if self.isTrain:
           ...
           # self.optimizer_D20 = torch.optim.Adam(self.netD20.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999))
           ...
           # self.optimizers.append(self.optimizer_D20)
    ...
    def backward_G(self):
        ...
        self.loss_G_L11_50 = self.criterionL1(self.fake_B50,  self.real_B50)
        ...
        self.loss_G_L11_100 = self.criterionL1(self.fake_B100,  self.real_B100)
        ...
        self.loss_G_L11_150 = self.criterionL1(self.fake_B150,  self.real_B150)

        x=torch.zeros(2,1,128,128)
        y=torch.zeros(2,1,128,128)


        self.fake_b50_0 = self.fake_B50*(self.mask0_0)
        self.fake_b0_0 = self.real_A*(self.mask0_0)
        ...
        self.loss_0_50 = self.criterionL1(torch.max(self.fake_b50_0-self.fake_b0_0+0.1,y.cuda(),out=None),x.cuda())
    ...
    def set_loss_G(self):
        self.backward_G()
        self.loss_G = torch.mean((100.0*self.loss_G_L11_50+self.loss_G_GAN_50) + (100.0*self.loss_G_L11_100+self.loss_G_GAN_100) + (100.0*self.loss_G_L11_150+self.loss_G_GAN_150)  +self.loss_50_100+self.loss_100_150+self.loss_0_50)
    ...
    def optimize_parameters(self):
        ...
        # update D
        self.set_requires_grad([self.netD50,self.netD100,self.netD150], True)  # enable backprop for D
        self.optimizer_D50.zero_grad()     # set D's gradients to zero
        self.optimizer_D100.zero_grad()     # set D's gradients to zero
        self.optimizer_D150.zero_grad()     # set D's gradients to zero

        self.backward_D50()                # calculate gradients for D
        self.backward_D100()                # calculate gradients for D
        self.backward_D150()                # calculate gradients for D

        self.optimizer_D50.step()          # update D's weights
        self.optimizer_D100.step()          # update D's weights
        self.optimizer_D150.step()          # update D's weights
